// --- ES Module Imports ---
import JSZip from './jszip.js'; // You must provide an ES module version or use a bundler.
import kDefaultSettings from './default-settings.js';
import PlaybackRateController from './playback-rate-controller.js';
// import console from './console.js'; // Usually not needed; use the browser's native console

// --- Feature Detection for Browser/Chrome ---
const BROWSER = (typeof browser !== 'undefined') ? 'firefox' : 'chrome';
const runtime = (typeof browser !== 'undefined') ? browser.runtime : chrome.runtime;

// --- Optional: Define VERSION ---
const VERSION = '1.0.0'; // Or inject via build tool

////////////////////////////////////////////////////////////////////////////////

// Hook JSON.parse to intercept manifest
...
// ... (history.pushState and history.replaceState hook unchanged) ...
...
let gRenderOptions = { ...kDefaultSettings };
let gSecondaryOffset = 0;

// Modern: Use the runtime API to get the extensionId
const extensionId = runtime.id;

function getMsgPort() {
  if (gMsgPort) return gMsgPort;

  gMsgPort = runtime.connect(extensionId);
  console.log(`Linked: ${extensionId}`);

  gMsgPort.onMessage.addListener(msg => {
    if (!msg.settings) return;
    gRenderOptions = { ...msg.settings };
...
// Connect with background script for settings
if (BROWSER === 'chrome') {
  try {
    getMsgPort();
  } catch (err) {
  ...
  window.addEventListener(
    ...
    evt => {
      ...
      if (evt.data.action === 'apply-settings' && evt.data.settings) {
        gRenderOptions = { ...evt.data.settings };
        gRendererLoop && gRendererLoop.setRenderDirty();
      }
    },
    false
...
window.__NflxMultiSubs = nflxMultiSubsManager; 
